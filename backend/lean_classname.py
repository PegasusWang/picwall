#!/usr/bin/env python
# -*- coding:utf-8 -*-

GIRLS_CLASS_NAME = {
    'Adnis':  ['sexy', 'girls', 'adult'],    # need check adult
    'Aoababofan': ['korean', 'girls', 'gif'],
    'Bestofasiangirls': ['sexy', 'asian', 'girls'],
    'Blendy99': ['sexy', 'girls', 'japan'],
    'Chinabeauties': ['sexy', 'china', 'girls'],
    'Ferchoecho': ['sexy', 'japan', 'girls', 'adult'],    # need check adult
    'Forchi': ['sexy', 'china', 'girls'],
    'Ganpukudou': ['beauty', 'girls', 'pure', 'china', 'japan'],
    'Gigachaoss': ['sexy', 'girls', 'japan', 'adult'],    # need check adult
    'Girl2chick': ['sexy', 'girls', 'japan', 'adult'],    # need check adult
    'Girls': ['sexy', 'girls', 'japan', 'korean'],

    'Heypantyhose': ['woman', 'china', 'girls', 'sexy'],
    'Happylim': ['sexy', 'girls', 'china', 'adult'],    # need check adult
    'Hotcosplaychicks': ['sexy', 'cosplay'],
    'HotGirlsAsia': ['sexy', 'asia', 'hot'],
    'HotGirlsfc': ['sexy', 'japan', 'hot', 'korean'],
    'Idolmaniax': ['sexy', 'japan', 'hot', 'korean'],
    'Jgiri': ['sexy', 'beauty', 'japan', 'korean', 'china', 'girls'],
    'Kawaiileg': ['leg', 'japan', 'sexy', 'korean'],

    'Koreangirlshd': ['beauty', 'sexy', 'korean', 'girls', 'models'],
    'Kormodels': ['beauty', 'sexy', 'korean', 'girls', 'models'],
    'KormodelsGif': ['gif', 'beauty', 'sexy', 'korean', 'girls', 'models'],
    'Legloveworld': ['leg', 'sexy', 'japan', 'girls'],
    'Lovelyasians': ['sexy', 'beauty', 'japan', 'korean', 'china', 'girls', 'models',
                     'lovely'],
    'Mzitu': ['china', 'sexy'],
    'PassionNippones': ['japan', 'sexy', 'adult'],    # need check adult
    'Silymarin': ['japan', 'sexy', 'asian'],    # need check
    'Snsdpics': ['korean', 'lovely', 'girls', 'models'],

    'Sossex1': ['hot', 'japan', 'sexy', 'girls'],    # need check
    'Sekkusu': ['hot', 'japan', 'korean', 'sexy', 'grils', 'adult'], # to check
    'SexyLadyJapan': ['sexy', 'japan', 'lady'],
    'Touch45': ['hot', 'asian', 'sexy', 'girls'],    # need check
    'Visualangel': ['hot', 'korean', 'girls', 'star'],

}


GIFS_CLASS_NAME = {
    'Aewae': ['funny', 'gif'],
    'Elmontajista': ['funny', 'gif'],
    'Gifsboom': ['gif', 'funny'],
    'Gifson': ['gif', 'funny'],
    'Icachondeo': ['gif', 'funny'],
    'ImgFile': ['gif', 'funny'],
    'Lolgifru': ['lol', 'gif', 'funny'],
    'TumblrForgifs': ['gif', 'funny'],
    'TumblrGifak': ['gif', 'funny'],
    'Tychoon': ['gif', 'funny'],

}

FASHION_CLASS_NAME = {
    'KoreanFashion': ['boys', 'fashion'],

}




BOYS_CLASS_NAME = {
    'Allboysboys': ['korean', 'handsome'],
    'Lobbu': ['boys', 'korean boy', 'handsome', 'gif'],


}


ANIMALS_CLASS_NAME = {
    'Althingscute':  ['animals', 'cute'],
    'Catsdogsblog': ['cats', 'dogs', 'funny', 'animals', 'cute'],
}
